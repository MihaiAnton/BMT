...
def calculate_metrics(
    ...
    for metric in evaluator.scores:
        ...
        metrics['Average across tIoUs'][metric] = sum(
            score) / float(len(score))
...
def greedy_decoder(model, feature_stacks, max_len, start_idx, end_idx, pad_idx, modality, nocuda: bool = False):
    ...
    with torch.no_grad():
        ...
        while (trg.size(-1) <= max_len) and (not completeness_mask.all()):
            ...
            completeness_mask = completeness_mask | torch.eq(
                next_word, end_idx).byte()
...
#     path_to_save = os.path.join(cfg.model_checkpoint_path, f'model_e{epoch}.pt')
    path_to_save = os.path.join(
        cfg.model_checkpoint_path, f'best_cap_model.pt')
...
def make_masks(feature_stacks, captions, modality, pad_idx):
    ...
    if modality == 'video':
        if captions is None:
            masks['V_mask'] = mask(feature_stacks['rgb']
                                   [:, :, 0], None, pad_idx)
        else:
            masks['V_mask'], masks['C_mask'] = mask(
                feature_stacks['rgb'][:, :, 0], captions, pad_idx)
    elif modality == 'audio':
        assert len(feature_stacks['audio'].shape) == 3
        if captions is None:
            masks['A_mask'] = mask(
                feature_stacks['audio'][:, :, 0], None, pad_idx)
        else:
            masks['A_mask'], masks['C_mask'] = mask(
                feature_stacks['audio'][:, :, 0], captions, pad_idx)
...
def training_loop(cfg, model, loader, criterion, optimizer, epoch, TBoard):
    ...
    for i, batch in enumerate(tqdm(loader, desc=progress_bar_name)):
        ...
        masks = make_masks(
            batch['feature_stacks'], caption_idx, cfg.modality, loader.dataset.pad_idx)
    ...
    if TBoard is not None:
        TBoard.add_scalar('debug/train_loss_epoch',
                          train_total_loss_norm, epoch)
...
def validation_1by1_loop(cfg, model, loader, decoder, epoch, TBoard):
    ...
    if phase == 'val_1':
        ...
        # no need to wait: they all the same as they are predicted for gt segments
        tIoUs = [0.5]
    elif phase == 'val_2':
    ...
    for i, batch in enumerate(tqdm(loader, desc=progress_bar_name)):
        ...
        list_of_lists_with_strings = [
            [loader.dataset.train_vocab.itos[i] for i in ints] for ints in ints_stack]
        ...
        list_of_lists_with_filtered_sentences = [
            None] * len(list_of_lists_with_strings)
    ...
    if cfg.log_path is None:
        ...
        with HiddenPrints():
            val_metrics = calculate_metrics(
                reference_paths, submission_path, tIoUs, cfg.max_prop_per_vid)
        ...
        if (TBoard is not None) and (phase != 'learned_props'):
            ...
            TBoard.add_scalar(
                f'{phase}/meteor', val_metrics['Average across tIoUs']['METEOR'] * 100, epoch)
            TBoard.add_scalar(
                f'{phase}/bleu4', val_metrics['Average across tIoUs']['Bleu_4'] * 100, epoch)
            TBoard.add_scalar(
                f'{phase}/bleu3', val_metrics['Average across tIoUs']['Bleu_3'] * 100, epoch)
            TBoard.add_scalar(
                f'{phase}/precision', val_metrics['Average across tIoUs']['Precision'] * 100, epoch)
            TBoard.add_scalar(
                f'{phase}/recall', val_metrics['Average across tIoUs']['Recall'] * 100, epoch)
            TBoard.add_scalar(f'{phase}/duration_of_1by1',
                              (time() - start_timer) / 60, epoch)
